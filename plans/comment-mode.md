...
- [ ] A submitted `comment`/`sign`/`sign_conditional` review sends `review-receipt-<ts>` to its author (reusing `notifications`'s existing `decline` listener via the new `submit` event; deferred from [`notifications`](notifications.md), PR #15).
