...
| `axi-skills` | any work on `packages/cli/` (the `drafter-axi` admin CLI): build pipeline, bash shim, SessionStart hook, generated SKILL.md, home vs dashboard views |
...
- **API**: Fastify 5 with a deny-by-default auth gateway. **Web**: React 19 + Vite + Tailwind v4 + React Router v7, built to static assets the API serves. **CLI**: AXI-style, shipped as a skill.
- **Storage**: a private git data repo of gitsheets records, single writer, push daemon. **No database.** Draft autosaves and open counts are write-behind; everything user-significant commits immediately with a readable subject and trailers.
