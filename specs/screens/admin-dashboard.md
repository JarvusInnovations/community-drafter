...
- Header with title, state, phase, both deadlines, and the **audience** — "Public — anyone with the link can read" or "Invitees only", and, when `list_visible_to` names organizations on a closed document, "signatory list also shared with *X*, *Y*" (`specs/data-model.md` § Audience). It is the promise the sign card makes to every signer, so the team reads it where they read the rest of the document's settings. Buttons: "Extend deadline…", "Copy public link" (if enabled), "Export feedback" (downloads the bundle from `behaviors/review-and-judgement.md`), "Export links" (CSV; recorded).
