...
3. **Status card** (the sign card), always above the document text:
   - *Not signed, commenting or signing phase*: heading "Add your name"; capacity choice if more than one allowed; name (prefilled); descriptor (personal) or organization and title (official — both required, `behaviors/signatures.md` § Capacity) per capacity; attestation checkbox for official; then, still above the button, the **who-sees sentence** and the **listing choice** (both below); the primary button **"Sign as Jane Doe"**, or **"Sign for St. Brigid Parish Council"** in official capacity; beneath it the reassurance line quoting `signing_closes_at`: "You can remove your name any time until Sep 30. We'll email you when the final version is published." Below the button, two quiet links: "I'd rather not sign" (decline) and "I have comments first" (opens comment mode).
     - *Who sees your name*: one sentence, derived from the document's audience (`../data-model.md` § Audience), its `show_signatories` setting and, on a closed document, `list_visible_to` (`behaviors/signatures.md` § Consent at signing). With `show_signatories = list`: public — "Your name will appear on the signatory list, which anyone with the link can read."; closed — "Your name will appear on the signatory list, which only the people invited to this document can see.", plus, when `list_visible_to` names organizations, a second sentence: "The team also shares the list with *St. Brigid Parish Council* and *City Arts Council*." With `count`: "Only the number of signatories is shown; your name goes to the team." With `none`: "No signatory list is shown for this document; your name goes to the team." The sentence never promises more privacy than the settings give.
