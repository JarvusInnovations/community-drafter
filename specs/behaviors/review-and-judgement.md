...
- Dispositions are fields on the comment record, written in the publish commit itself (`Comments` trailer lists the ids), so the version that answered is the commit that set them.
...
The admin export for a document produces, in one file: the current version's text; every submitted comment not yet dispositioned, with author display name, capacity/org, judgement of the review it came from, anchor (quote, heading path, block), body, and version; a separate, labeled section of **unsubmitted** saved comments with the same fields minus judgement; a tally of judgements per version; and the current signatory count. The export is the input to the team's or agent's revision work; the publish action is the output path. Dispositions may target unsubmitted comments too; the author sees the disposition if they later submit or on their own page.
