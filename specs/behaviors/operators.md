...
- If the address matches an active operator, a **magic link** is emailed, built on the hostname the request arrived on and naming that site. It is valid for 15 minutes and carries its own proof: a short code that signs the site, the operator, the expiry and the return path with `AUTH_SECRET` (`api/auth.md`). Nothing about it is kept in memory, so a link followed after the service has scaled to zero and started again still works. Following it sets the web session cookie and redirects to the requested return path (validated: must start with a single `/`). A used link is remembered in memory until it expires; reuse after a restart within the window is accepted as a known limitation.
...
3. The CLI polls `POST /auth/device/token` with the `device_code` every 3 seconds for up to 15 minutes; once approved it receives the 90-day CLI token and writes it to the profile. Pending device codes live in memory; the polling itself keeps the service from going idle while one is pending, and a restart before approval (a deploy) fails the login and the CLI says so.
...
Because the service is the data repository's single writer, changes pushed to the remote from elsewhere are not visible until the service refreshes. `POST /admin/api/refresh`, authenticated by an HMAC signature over the body with `DATA_REPO_WEBHOOK_SECRET` (GitHub's `X-Hub-Signature-256`), makes the service: acquire the write lock, confirm no commits are waiting to be pushed (otherwise respond 409 `refresh_busy` and let the caller retry), `git fetch`, fast-forward the local branch to the remote (a non-fast-forward is refused with 409 `refresh_diverged` and logged), rebuild the read model, release the lock. There is no periodic fetch. A webhook that arrives while the service is scaled to zero starts it, and the boot clone already holds the pushed change (`architecture.md` § Deployment). This is how a hand edit made with `gitsheets-axi` and pushed to the remote reaches the running instance.
