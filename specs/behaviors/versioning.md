...
**Publishing.** An admin (human or agent) publishes a version by supplying the full new markdown text, a summary (1–200 characters, imperative or descriptive, e.g. "Tightened term 2; added the collections-care ask"), optional team notes, and optional dispositions for earlier comments (see `review-and-judgement.md`). Publishing is one transaction and therefore one commit: subject `publish: <slug> v<n>`, trailers `Summary:`, `Notes:` (when given), `Disposed:` (when dispositions are attached), `Document:`, `Actor:`; the commit writes the document body, any settings it changes, and the disposition fields on the affected submissions together. Publishing notifies nobody unless the operator asks: with `--notify-commenters` the authors whose comments this publish answered are told (`notifications.md` → `disposition-v<n>`), and without it the response says how many would have been. A publish whose text is byte-identical to the current version is refused (`no_change`) so a version always changes something. Version numbers are never reused or skipped because the history is append-only.
...
**The label.** Wherever a version's text is shown, its label reads: "**Version 3** · published Sep 20, 2026 at 9:14 AM EDT · *Tightened term 2; added the collections-care ask*". If it is not the latest, a notice says so and links to the latest. Numbers, dates and the summary are derived from the commit; nothing about git appears.

**No version is "final".** The team may publish until the statement is delivered; the text of record is simply the current version when signing closes or the document is delivered. A version stored by an earlier build with a `Final: true` trailer reads like any other, and the trailer is ignored.
...
1. Split each version's rendered text into the units a reader would name: paragraphs, headings, list items, whole tables and whole code blocks. A table is one unit, not a loose run of cells; its cells are only ever compared against the cells of the table it aligns with. A fenced code block is one unit too, compared on its exact text; one written inside a list item is part of that list item and is not a unit of its own. Code blocks are comparison units without being commentable blocks (`inline-comments.md` § Block identity), so comparing them changes no block's id or text.
...
5. A changed code block is never redlined word by word — a word diff of code reads as noise. It is shown stacked like a restructured table: the whole old block struck through and labelled as removed, above the whole new block underlined and labelled as added. An unchanged code block is shown once.
6. Whole added or removed units are rendered whole with the same styling.
7. Formatting-only changes (heading level, list marker) are shown as a small marginal note, not as a redline of the whole unit.

The comparison view carries a summary line and a toggle to hide unchanged units. The summary counts changes the way a reader would: one reworded paragraph is one change, and one edited table is one change however many cells it touched. It names the kinds involved rather than reporting bare totals — "2 paragraphs changed, 1 table changed", "1 heading changed, 2 list items added", "1 code block changed" — listing changed kinds first, then added, then removed, omitting any kind with nothing to report, and reading "No changes" when the two versions render identically. Default comparison is latest vs previous; any pair may be selected.
...
**Local**
- **One publish, one commit.** The commit is the version: text, summary, notes and dispositions ride together or not at all. Implementers must not write the text in one commit and its metadata in another, and must not keep a versions table that could disagree with the history.
