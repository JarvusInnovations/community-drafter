...
- `POST /documents/:slug/schedule` `{ comments_close_at?, signing_closes_at?, notify?: boolean, dry_run?: boolean }` → extension only; `deadline_not_later` otherwise; `no_deadline_set` (422) when the document has no such deadline to extend, with a message naming the deadline in words and pointing at `POST /documents/:slug/open` rather than reporting that a stored field must move later. Records the change. Sends `schedule-changed-<ts>` to the O segment **only with `notify: true`** (`behaviors/notifications.md`). The response is the document summary plus `deadlines: [{ deadline, from?, to }]` and `notify: { requested, would_notify, sent?, failed?, failures? }` — `would_notify` is always the size of O, so a call without `notify` still says whom it did not tell. With `dry_run` nothing is written or sent and the response is `{ dry_run: true, deadlines, notify: { requested, would_notify } }`, after the same validation.
- `POST /documents/:slug/close` → closes now (sets `signing_closes_at = now`); rare, recorded; sends nothing.
- `POST /documents/:slug/reopen` `{ comments_close_at?, signing_closes_at, notify?, dry_run? }` → same `notify` and `dry_run` semantics and response as `schedule`.
- `POST /documents/:slug/confirm-call` `{ by?, dry_run? }` → asks S-behind and C to keep or remove their name (`behaviors/notifications.md` → `confirm-call-<ts>`). `by` is ISO 8601 with a zone and defaults to `signing_closes_at`; it must be in the future and no later than `signing_closes_at` (422 `validation_failed`, `field: "by"`). Allowed in the commenting and signing phases (409 `phase_closed` otherwise) and before delivery (409 `already_delivered` after). Response `{ by, sent, failed, failures: [{ person, error }], commit }`, where `commit` is the subject of the one `Action: confirm-call` commit marking the people reached, `null` when nobody was. With `dry_run`, `{ dry_run: true, by, would_send: [{ person, name, reason: "behind" | "conditional", signed_on_version }] }`. Nobody qualifying is not an error: `sent: 0`, `commit: null`, and nothing is written.
- `POST /documents/:slug/delivered` `{ note?, dry_run? }` → records delivery (`behaviors/signatures.md` § Delivery): one `Action: deliver` commit setting `delivered_at` (now) and `delivered_note`, then `delivered` to every current signer, whose `notified.delivered` marks are written in a follow-up send commit. Allowed once signing has opened, in the signing or closed phase (409 `phase_closed` before, and for a withdrawn document); 409 `already_delivered` when `delivered_at` is already set. Response: the document summary plus `{ sent, failed, failures }`. With `dry_run`, `{ dry_run: true, would_send: n }` and nothing is written.
...
- `POST /documents/:slug/versions`
  ...
  ```
  { body, summary, notes?, notify_commenters?: boolean,
