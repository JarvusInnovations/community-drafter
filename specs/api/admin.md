...
- `PATCH /sites/:slug` `{ name?, sender_name?, sender_email?, reply_to?, logo_url?, accent? }` → updates (`Action: site-update`). `hostname` is not patchable: a hostname change is a new site, because DNS, a certificate and every link already sent are attached to the old one.
- `GET /sites/:slug/operators` → the site's group.
- `POST /sites/:slug/operators` `{ email }` → adds an operator to the group (`Action: site-operator-add`), creating the record when the email is new; 422 when the operator is not active.
