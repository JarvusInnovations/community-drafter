...
- `POST /documents` `{ slug, title, capacities?, audience?, public_access?, show_signatories?, list_visible_to?, sender_name, reply_to, revocation_window_hours?, tags? }` → document (state `draft`) with `created_by` and `operators = [caller]`. `audience` is `public` | `closed` (default `closed`) and is the spelling this API prefers: it writes `public_access` (`public` → `read`, `closed` → `none`), which is where the audience lives (`data-model.md` § Audience). `public_access` may still be passed directly for the phase-2 `participate` value; passing both with different meanings is 422 `validation_failed`. `list_visible_to` is an array of organization names, meaningful only on a closed document.
...
- `GET /documents/:slug` → document + versions (from content history) + counts. Every document shape this API returns carries `audience` derived from `public_access` beside it, so a consumer never has to do the mapping itself and can never disagree with the CLI or the dashboard about it. The signature counts carry `behind`: live signatures attached to a version older than the current one (`behaviors/signatures.md` § A signature belongs to a version).
- `PATCH /documents/:slug` → settings fields only (not `state`, not deadlines); accepts `audience` and `list_visible_to` on the same terms as `POST /documents`.
