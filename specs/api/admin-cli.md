...
- **`docs export` never writes a PDF to stdout.** It is the one command whose payload is binary; it always lands in a file, and what the terminal gets is the path, the version, the counts and the draft-or-clean word. An agent that wants the numbers reads those; an operator who wants the document opens the file.
