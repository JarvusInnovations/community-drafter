...
- **A people view names the site its people belong to.** `people list` and `people import` print the document's `site` alongside the rows, because an email is unique within a site and not across the instance (`specs/behaviors/sites.md` § People are per site): the same address on two sites is two people, and an operator reading a list has to know which one they are looking at.
