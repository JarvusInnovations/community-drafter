...
| `docs show <slug>` | dashboard numbers (including how many signatures are behind the current version, how many signers a confirm-call would reach, and how many invitees are unopened or undecided), `delivered_at` once delivered, the audience and who the statement is addressed to, the **site** and the canonical hostname its personal and public links are built on, versions, schedule |
...
| `docs extend <slug> [--comments-close <when>] [--signing-closes <when>] [--notify] [--dry-run]` | extension. Prints each deadline's old and new time and **how many opened-but-undecided people** an announcement would reach. Without `--notify` nobody is told and the output says so, with the count; with `--notify` they get `schedule-changed` and the output reports delivered and failed. `--dry-run` validates and prints the same, writing and sending nothing |
| `docs confirm-call <slug> [--by <when>] [--dry-run]` | ask every signer whose signature is behind the current version, and every conditional signer, to keep or remove their name by `--by` (default: when signing closes). Prints how many were asked and the commit subject, or that nobody needs to confirm; `--dry-run` lists who would be asked and why (`behind v2` / `conditional`) |
| `docs delivered <slug> [--note "…"] [--dry-run]` | record that the statement was delivered and tell every current signer; once per document (a second run is refused with `already_delivered`). Prints `delivered_at`, the note, delivered and failed counts; `--dry-run` prints how many signers would be told |
...
| `docs close | reopen | withdraw <slug> …` | lifecycle; `reopen` takes `--notify` and `--dry-run` exactly as `extend` does, and `close` sends nothing |
| `docs export <slug> --pdf [--out <file>] [--paper letter\|a4] [--citations links\|footnotes\|hybrid] [--draft]` | write the deliverable (`specs/screens/deliverable.md`) to a file: the current version's text, its title block, and the signatory list as it stands. `--pdf` names the format and is the only one today. Without `--out` the file is `<slug>-v<n>.pdf` in the working directory, or `<slug>-v<n>-draft.pdf` while the deliverable is still the watermarked form; the command prints the path it wrote, the version, the paper, the size, and whether the copy is a draft or clean — and nothing about who signed, because the counts belong to `signatures list` and a second request to fetch them could only report a moment other than the one the file was rendered at. `--citations` picks how the citation links in the text are presented (`specs/behaviors/versioning.md` § Citations); the default is `hybrid`, the form that serves a reader on a screen and a reader holding the page. `--draft` forces the watermarked form of a document that has already gone clean; there is no flag the other way |
| `versions list <slug>` / `versions show <slug> <n> [--body]` | read |
| `versions publish <slug> --file new.md --summary "…" [--notes-file …] [--dispositions d.json] [--notify-commenters]` | publish: one commit whose trailers carry the summary; prints version number, commit subject, `signing_closes_at` only when the publish moved it (a `null` line reads as a deadline that was cleared), and how many answered commenters would be told — and whether they were: nobody is mailed without `--notify-commenters`. There is no `--final`: the team may publish until delivery |
...
| `people remind <slug> --target unopened\|opened-not-acted [--min-age <hours>] [--dry-run]` | reminders — the last call, naming the next deadline and asking them to sign or decline; skips anyone this document messaged within `--min-age` hours (default 48, `0` to send regardless) and prints what it actually sent, how many were skipped as recently messaged and how many by preference |
...
| `signatures list <slug> [--include-revoked] [--conditional]` | signatures, each with the version it is attached to, whether it is behind the current version, and whether the signer is on the signatory list (`listed`) |
...
`SKILL.md` carries a **What participants are emailed** section: the message matrix of `behaviors/notifications.md` condensed to one line per message (who it reaches and which command sends it), and the rule that nothing reaches participants unless a person acted or an operator ran a command with its flag. An agent driving a document must know that `docs extend` and `versions publish` are silent by default.
